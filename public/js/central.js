...
// Guarda IDs já vistos para não repetir toast
const seen = new Set()

// Evitar "chuva" de toasts no primeiro carregamento
let firstRun = true

// Pausa o polling quando a aba não está visível (economia de rede/CPU)
let pollingOn = true
...
// Wrapper de fetch com tratamento simples de erro
...
// Carrega reclamações PENDENTES e monta os cards por marketplace
async function carregarReclamacoesAbertas() {
  ...
  // Dispara toast apenas para IDs NOVOS (e não no primeiro carregamento)
  rows.forEach((r) => {
    const id = String(r.id)
    if (firstRun) {
      // Semear os atuais para não disparar spam na 1ª visita
      seen.add(id)
      return
    }
    if (!seen.has(id)) {
      ...
      showToast({
        ...
        href: `/devolucao-editar.html?id=${encodeURIComponent(id)}`
      })
    }
  })
}

// Monta “Devoluções a caminho” (prioriza recebido_cd → em_inspecao → pendente)
...
async function boot() {
  ...
  while (true) {
    ...
    try {
      ...
      // Silencia falhas temporárias de rede/servidor
