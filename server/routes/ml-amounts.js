...
// fetch fallback (Node 18+ tem nativo)
...
async function mget(token, path, { timeout = 12000 } = {}) {
  ...
  try {
    ...
    if (!r.ok) {
      ...
      e.status = r.status; e.payload = body; throw e;
    }
    return body;
...
function labelFromReasonKey(key){
  ...
  if (['cliente_endereco_errado','wrong_address_buyer','recipient_absent','absent_receiver','didnt_pickup'].includes(k)) return 'Cliente: endereço errado';
  if (['produto_defeito','product_defective','broken','damaged','incomplete','missing_parts','quality_issue'].includes(k)) return 'Produto com defeito';
  if (['avaria_transporte','damaged_in_transit','shipping_damage','carrier_damage'].includes(k)) return 'Avaria no transporte';
...
function labelFromReasonName(name='') {
  ...
  if (has('endereco errado') || has('endereco incorreto') || has('ausencia') || has('destinatario ausente') || has('wrong address') || has('absent')) return 'Cliente: endereço errado';
  if (has('defeit') || has('avari') || has('danific') || has('quebrad') || has('faltando') || has('incomplet') || has('missing') || has('broken') || has('damaged')) return 'Produto com defeito';
  if (has('transporte') || has('shipping damage') || has('carrier damage')) return 'Avaria no transporte';
  if (has('pedido incorreto') || has('produto errado') || has('wrong item') || has('not as described') || has('produto diferente') || has('modelo diferente') || has('tamanho diferente') || has('cor diferente') || has('produto trocado')) return 'Pedido incorreto';
  return null;
}
...
const sumOrderProducts = (o) => {
  ...
  for (const it of getOrderItems(o)) {
    const unit = toNumber(it?.unit_price ?? it?.full_unit_price ?? it?.sale_price ?? it?.price);
...
function computeReasonExtras(claim) {
  ...
  const reason_label =
    ...
    (reasonName && labelFromReasonName(reasonName)) || null;
  return { reason_label };
}
...
module.exports = function registerMlAmounts(app) {
  ...
  app.get('/api/ml/returns/:id/fetch-amounts', async (req, res) => {
    ...
    try {
      ...
      if (!pool.length) return res.status(400).json({ error: 'Nenhum access token disponível (verifique ml_accounts/ml_tokens ou variáveis MELI_TOKEN_*)', meta });
      ...
      return res.json({
        ...
        amounts, order, claim,
        reason_code: (claim && claim.reason_id) || null,
        reason_name: (claim && claim.reason_name) || null,
        reason_key:  (claim && claim.reason_key) || null,
        reason_label,
  ...
  // ---------- ENRICH (no-op, registra evento se houver tabela) ----------
