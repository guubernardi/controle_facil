...
class DevolucoesFeed {
  constructor() {
    ...
    // Tempo para considerar "nova devolução" (em horas)
    this.NEW_HOURS = 4;
  ...
  async carregar() {
    ...
    try {
      ...
      this.items.sort((a, b) => {
        const da = new Date(a.created_at || a.updated_at || a.cd_recebido_em || a.data_compra || 0).getTime();
        const db = new Date(b.created_at || b.updated_at || b.cd_recebido_em || b.data_compra || 0).getTime();
        return db - da;
      });
  ...
  criarCard(d) {
    ...
    // Fonte da data:
    // 1) created_at da devolução
    // 2) updated_at / cd_recebido_em
    // 3) data_compra do pedido (fallback)
    const dataFonte =
      d.created_at ||
      d.updated_at ||
      d.cd_recebido_em ||
    ...
    // Nova devolução? (últimas NEW_HOURS horas)
    let novaBadge = "";
    if (dataFonte) {
      const dt = new Date(dataFonte);
      if (!Number.isNaN(dt.getTime())) {
        const diffHoras = (Date.now() - dt.getTime()) / 36e5;
        if (diffHoras <= this.NEW_HOURS) {
          novaBadge = '<span class="badge badge-nova">Nova</span>';
        }
      }
    }

    // Valores com fallback (caso você mande com outros nomes)
    const rawProd  = d.valor_produto ?? d.produto_valor ?? d.product_amount ?? d.amount_produto;
    const rawFrete = d.valor_frete   ?? d.frete_valor   ?? d.shipping_amount ?? d.amount_frete;

    const valorFmt = Number(rawProd  || 0).toLocaleString("pt-BR", { style: "currency", currency: "BRL" });
    const freteFmt = Number(rawFrete || 0).toLocaleString("pt-BR", { style: "currency", currency: "BRL" });
  ...
  traduzirStatusML(st) {
    const map = {
      ...
      closed:         "Encerrado",
      expired:        "Expirado",
      label_generated:"Pronto para envio" // etiqueta gerada, pronto pra postar
    };
    return map[st] || st || "—";
  }
  ...
  getClassStatusML(st) {
    ...
    if (st === "shipped" || st === "label_generated") return "badge-status-neutro";
