...
module.exports = function registerMlSync(app, opts = {}) {
  ...
  router.get('/api/ml/claims/import', async (req, res) => {
    ...
    try {
      ...
      // checar colunas disponíveis (robusto)
      const colInfo = await tableHasColumns('devolucoes', [
        'tipo_reclamacao', 'foto_produto', 'valor_produto', 'valor_frete',
        'log_status', 'ml_return_status', 'updated_at', 'sku', 'loja_nome', 'ml_claim_id', 'status'
      ]);
      ...
      const hasLogCol    = !!colInfo.log_status;
      const hasMlRetCol  = !!colInfo.ml_return_status;
      const hasUpdCol    = !!colInfo.updated_at;
      const hasSkuCol    = !!colInfo.sku;
      const hasLojaCol   = !!colInfo.loja_nome;
      const hasClaimCol  = !!colInfo.ml_claim_id;
      const hasStatusCol = !!colInfo.status;
        ...
        for (const [claimId, it] of claimMap.entries()) {
          ...
          try {
            ...
            if (!dry) {
              ...
              if (hasLogCol)    { set.push(`log_status = COALESCE($${params.length+1}, log_status)`);      params.push(flow); }
              if (hasMlRetCol)  { set.push(`ml_return_status = COALESCE($${params.length+1}, ml_return_status)`); params.push(String(ret?.status || '')); }
              if (hasStatusCol) { set.push(`status = COALESCE($${params.length+1}, status)`);               params.push(internal); }
              if (hasSkuCol)    { set.push(`sku = COALESCE($${params.length+1}, sku)`);                     params.push(sku); }
              if (hasLojaCol)   {
                set.push(`loja_nome = CASE
                  WHEN (loja_nome IS NULL OR loja_nome = '' OR loja_nome = 'Mercado Livre')
                    THEN COALESCE($${params.length+1}, loja_nome)
                  ELSE loja_nome
                END`);
                params.push(lojaNome);
              }
              if (hasClaimCol)  { set.push(`ml_claim_id = COALESCE($${params.length+1}, ml_claim_id)`);     params.push(String(claimId)); }
              if (hasFotoCol)   { set.push(`foto_produto = COALESCE($${params.length+1}, foto_produto)`);   params.push(fotoUrl); }
              if (hasProdCol)   { set.push(`valor_produto = COALESCE($${params.length+1}, valor_produto)`); params.push(prodAmount); }
              if (hasFreteCol)  { set.push(`valor_frete = COALESCE($${params.length+1}, valor_frete)`);     params.push(freightAmount); }
              if (hasUpdCol)    { set.push(`updated_at = now()`); }
      ...
      const detail = e?.response?.data || e?.message || String(e);
      // Mesmo em falha geral, evita 400 para não quebrar o front.
